...
EXTERN_C SNES_Screen
EXTERN_C ScreenX,ScreenY
EXTERN_C SCREEN_MODE
EXTERN_C SNES_Screen8
EXTERN_C Copy_Screen
EXTERN_C HICOLOUR_Palette
...
EXPORT Clear_Screen
    ...
    mov edi,[C_LABEL(SNES_Screen)]
    xor eax,eax

    mov ecx,[C_LABEL(ScreenX)]
    shr ecx,2
    imul ecx,[C_LABEL(ScreenY)]
ALIGNC
.clear_loop:
    sub ecx,byte 4
    mov [edi+ecx],eax
...
ALIGNC
EXPORT_C Copy_Screen_obsolete
    pusha

    mov al,[C_LABEL(SCREEN_MODE)]
    and eax,byte 7
...
MODEX_COPY:
    popa
    jmp C_LABEL(Copy_Screen)    ; Have Allegro blit the screen and ret for us
...
.pixel_loop:
    mov al,[esi]
    inc esi
    mov bx,[C_LABEL(HICOLOUR_Palette)+eax*4]
    mov [edi],bx
    add edi,byte 2
    dec dh
...
.pixel_loop:
    mov al,[esi]
    inc esi
    mov ebx,[C_LABEL(HICOLOUR_Palette)+eax*4]
    mov [edi],ebx
    mov [640*2+edi],ebx
    add edi,byte 4
    dec dh
